...
const SourcePanel = () => {
  ...
  const [fileName, setFileName] = useAddonState(
    `${ADDON_ID}/fileName`,
    params.files[0]
  );
  const [hideLineNumbers, setHideLineNumbers] = useAddonState(
    `${ADDON_ID}/hideLineNumbers`,
    !!params.hideLineNumbers
  );

  let fileMap = params.files.reduce(
    (prev, curr) => ({ ...prev, [curr.fileName]: curr }),
    {}
  );
...
function getParams() {
  ...
  if (p.allowedExtensions) {
    params.files = params.files.filter(
      (x) => p.allowedExtensions.indexOf(getExtension(x.fileName)) > -1
    );
    params.files = params.files.sort(
      (x, y) =>
        p.allowedExtensions.indexOf(getExtension(x.fileName)) -
        p.allowedExtensions.indexOf(getExtension(y.fileName))
    );
  }
